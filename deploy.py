...
def main(img_path=None, vid_path=True,vid_out = True):

    print(f"[INFO] Loading model... ")
    ## loading the custom trained model
    # model =  torch.hub.load('ultralytics/yolov5', 'custom', path='last.pt',force_reload=True) ## if you want to download the git repo and then run the detection
    model =  torch.hub.load('./yolov5-main', 'custom', source ='local', path='last_v5m.pt',force_reload=True) ### The repo is stored locally
...
#main(vid_path="./test_img_vid/lpra.mp4",vid_out="detec_vid.mp4") ### for custom video
# main(vid_path=0,vid_out="webcam_facemask_result.mp4") #### for webcam

main(img_path="./test_img_vid/test1.jpg") ## for image
